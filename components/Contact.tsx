...
export default function Contact() {
  ...
  return (
    <div id='contact' ref={ref} className='w-full bg-rose-300'>
      <div className="w-full">
        <svg className={`${scrollY < 600 ? 'lg:h-48 h-16' : 'h-0'} w-full transition-all duration-500`} data-name="Layer 1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 120" preserveAspectRatio="none">
            <path d="M649.97 0L550.03 0 599.91 54.12 649.97 0z" className="fill-neutral-200 dark:fill-neutral-600"></path>
        </svg>
      </div>
          ...
          <div className='flex flex-col items-center w-full'>
            ...
            <div style={{transform: `translateY(${scrollY < 500 ? 0 : -100}%)`, opacity: scrollY < 500 ? 1 : 0, pointerEvents: scrollY < 500 ? "all" : "none"}} className='flex flex-col lg:w-96 w-3/4 transition duration-500 mt-2'>
              <div className='flex w-full'>
                <label className='translate-y-2 bg-neutral-200 dark:bg-neutral-600 dark:text-neutral-50 px-6 py-1 rounded-full w-fit ml-4 font-semibold'>Name</label>
              ...
              <div className={`bg-neutral-100 dark:bg-neutral-500 rounded-xl w-full shadow-xl overflow-hidden ${nameInput.error && "border-red-600 border-2"}`}>
                <input className='w-full p-3 dark:text-neutral-50' name='name' placeholder='Your name here' value={nameInput["value"]} onChange={(e) => setNameInput(nameInput => ({...nameInput, value: e.target.value}))}/>
              </div>
            </div>
            {/* Email */}
            <div style={{transform: `translateY(${scrollY < 500 ? 0 : -100}%)`, opacity: scrollY < 500 ? 1 : 0}} className='flex flex-col lg:w-96 w-3/4 transition duration-500 delay-150 mt-2'>
              <div className='flex w-full'>
                <label className='translate-y-2 bg-neutral-200 dark:bg-neutral-600 dark:text-neutral-50 px-6 py-1 rounded-full w-fit ml-4 font-semibold'>Email</label>
              ...
              <div className={`bg-neutral-100 dark:bg-neutral-500 rounded-xl w-full shadow-xl overflow-hidden ${emailInput.error && "border-red-600 border-2"}`}>
                <input className='w-full p-3 dark:text-neutral-50' name='email' placeholder='Your email here' value={emailInput["value"]} onChange={(e) => setEmailInput(emailInput => ({...emailInput, value: e.target.value}))}/>
              </div>
            </div>
            {/* Subject */}
            <div style={{transform: `translateY(${scrollY < 500 ? 0 : -100}%)`, opacity: scrollY < 500 ? 1 : 0}} className='flex flex-col lg:w-96 w-3/4 transition duration-500 delay-300 mt-2'>
              <div className='flex w-full'>
                <label className='translate-y-2 bg-neutral-200 dark:bg-neutral-600 dark:text-neutral-50 px-6 py-1 rounded-full w-fit ml-4 font-semibold'>Subject</label>
              ...
              <div className={`bg-neutral-100 dark:bg-neutral-500 rounded-xl w-full shadow-xl overflow-hidden ${subjectInput.error && "border-red-600 border-2"}`}>
                <input className='w-full p-3 dark:text-neutral-50' name='subject' placeholder='The subject here' value={subjectInput["value"]} onChange={(e) => setSubjectInput(subjectInput => ({...subjectInput, value: e.target.value}))}/>
              </div>
            </div>
            {/* Message */}
            <div style={{transform: `translateY(${scrollY < 500 ? 0 : -50}%)`, opacity: scrollY < 500 ? 1 : 0}} className='flex flex-col lg:w-96 w-3/4 transition duration-500 delay-500 mt-2'>
              <div className='flex w-full'>
                <label className='translate-y-2 bg-neutral-200 dark:bg-neutral-600 dark:text-neutral-50 px-6 py-1 rounded-full w-fit ml-4 font-semibold'>Message</label>
              ...
              <div className={`bg-neutral-100 dark:bg-neutral-500 rounded-xl w-full shadow-xl overflow-hidden ${messageInput.error && "border-red-600 border-2"}`}>
                <textarea className='p-3 w-full dark:text-neutral-50' rows={5} name='message' placeholder='The message here' value={messageInput["value"]} onChange={(e) => setMessageInput(messageInput => ({...messageInput, value: e.target.value}))}/>
              </div>
            </div>
            <input className='bg-neutral-50 dark:bg-neutral-500 w-32 text-center cursor-pointer mt-16 px-8 py-3 rounded-full text-xs text-neutral-700 dark:text-neutral-50 font-bold lg:text-base lg:py-4 lg:px-12 hover:shadow-xl hover:-translate-y-1 transition' type="submit" value="Send Message" />
          </div>
        </form>
        }
        <div className='lg:w-1/2 w-3/4 overflow-hidden mb-16'>
          <h1 style={{transform: `translateY(${scrollY < 600 ? 0 : -120}%)`, transitionDelay: `${scrollY < 400 ? 100 : 0}ms`}} className='transition-transform duration-500 h-fit lg:text-7xl text-5xl font-bold  text-neutral-50 dark:text-neutral-600'>Contact Me</h1>
          <p style={{transform: `translateY(${scrollY < 600 ? 0 : -430}%)`}} className='leading-normal lg:text-2xl lg:mt-8 mt-4 transition-transform duration-500  text-neutral-200 dark:text-neutral-500'>For additional <span className='font-semibold'>information</span> or <span className='font-semibold'>freelancing requests</span> please send me an email</p>
        </div>
      </div>
    </div>
  )
