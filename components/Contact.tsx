  ...
  return (
    <div id='contact' ref={ref} className='w-full bg-purple-400 bg-opacity-30'>
      <div className="w-full">
        <svg className={`${scrollY < 600 ? 'md:h-48 h-16' : 'h-0'} w-full transition-all duration-500`} data-name="Layer 1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 120" preserveAspectRatio="none">
            <path d="M649.97 0L550.03 0 599.91 54.12 649.97 0z" className="fill-neutral-200"></path>
        </svg>
      </div>
      <div className='flex lg:flex-row flex-col-reverse justify-between items-center w-full md:px-32 pb-32'>
        {sendStatus === 'success' ?
        ...
        <form style={{opacity: sendStatus === "success" ? 0 : 1}} onSubmit={SendEmail} className='lg:w-1/2 w-full transition-all duration-500'>
          <div className='flex flex-col items-center w-full'>
            {/* Name */}
            <div style={{transform: `translateY(${scrollY < 500 ? 0 : -100}%)`, opacity: scrollY < 500 ? 1 : 0, pointerEvents: scrollY < 500 ? "all" : "none"}} className='flex flex-col md:w-96 w-3/4 transition duration-500'>
            ...
            {/* Email */}
            <div style={{transform: `translateY(${scrollY < 500 ? 0 : -100}%)`, opacity: scrollY < 500 ? 1 : 0}} className='flex flex-col md:w-96 w-3/4 transition duration-500 delay-150'>
            ...
            {/* Subject */}
            <div style={{transform: `translateY(${scrollY < 500 ? 0 : -100}%)`, opacity: scrollY < 500 ? 1 : 0}} className='flex flex-col md:w-96 w-3/4 transition duration-500 delay-300'>
            ...
            {/* Message */}
            <div style={{transform: `translateY(${scrollY < 500 ? 0 : -50}%)`, opacity: scrollY < 500 ? 1 : 0}} className='flex flex-col md:w-96 w-3/4 transition duration-500 delay-500'>
            ...
            <input className='bg-neutral-50 w-32 text-center cursor-pointer mt-16 px-8 py-3 rounded-full text-xs text-neutral-700 font-bold md:text-base md:py-4 md:px-12 hover:shadow-xl hover:-translate-y-1 transition' type="submit" value="Send Message" />
          </div>
        </form>
        }
