    ...
    <div id='contact' ref={ref} className='flex lg:flex-row flex-col-reverse justify-between w-full p-32'>
      ...
      <div className='lg:w-1/2 w-full h-96 flex flex-col items-center'>
        <div className='bg-neutral-100 border-l-8 border-green-600 rounded-xl h-24 w-96 mt-16 flex items-center py-2 px-4'>
            ...
            <button onClick={() => Reset()} className='absolute w-80 ml-4 translate-y-16'>Send Another</button>
          </div> 
      </div>
      :
      <form style={{opacity: sendStatus === "success" ? 0 : 1}} onSubmit={SendEmail} className='lg:w-1/2 w-full transition-all duration-500'>
        <div className='flex flex-col items-center'>
          ...
          {/* Email */}
          <div style={{transform: `translateY(${scrollY < 500 ? 0 : -100}%)`, opacity: scrollY < 500 ? 1 : 0}} className='flex flex-col w-96 transition duration-500 delay-150'>
          ...
          {/* Subject */}
          <div style={{transform: `translateY(${scrollY < 500 ? 0 : -100}%)`, opacity: scrollY < 500 ? 1 : 0}} className='flex flex-col w-96 transition duration-500 delay-300'>
          ...
          {/* Message */}
          <div style={{transform: `translateY(${scrollY < 500 ? 0 : -50}%)`, opacity: scrollY < 500 ? 1 : 0}} className='flex flex-col w-96 transition duration-500 delay-500'>
          ...
          <input className='bg-gradient-to-r from-purple-400 to-pink-600 w-32 text-center cursor-pointer mt-16 px-8 py-3 rounded-full text-xs text-neutral-50 font-bold md:text-base md:py-4 md:px-12 hover:shadow-xl hover:-translate-y-1 transition' type="submit" value="Send Message" />
        </div>
      </form>
      }
      <div className='lg:w-1/2 w-full overflow-hidden'>
        <h1 style={{transform: `translateY(${scrollY < 600 ? 0 : -120}%)`, transitionDelay: `${scrollY < 400 ? 100 : 0}ms`}} className='transition-transform duration-500 h-fit lg:text-7xl text-5xl font-bold'>Contact Me</h1>
        <p style={{transform: `translateY(${scrollY < 600 ? 0 : -430}%)`}} className='leading-normal lg:text-2xl lg:mt-8 mt-4 transition-transform duration-500'>For additional <span className='font-semibold'>information</span> or <span className='font-semibold'>freelancing requests</span> please send me an email</p>
      </div>
    </div>
  )
}
