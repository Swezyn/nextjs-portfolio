...
export default function Intro() {
  ...
  return (
    <div id='intro' ref={ref} className='w-full bg-neutral-100 h-fit flex items-center flex-col pt-48'>
        ...
        <h3 style={{transform: `translateY(${scrollY < -200 ? 50 : 0}%)`, opacity: -scrollY * 0.003 + 1}} className='font-bold uppercase text-base tracking-widest leading-loose md:text-2xl text-neutral-700 transition-transform duration-500 ease-linear'>Hi, My name is</h3>
        <h1 className='font-extrabold text-4xl leading-normal text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-pink-600 md:text-7xl md:leading-normal'>Elias Akesson</h1>
        <div style={{transform: `scaleX(${scrollY < 0 ? 1.5 : 1}) translateY(${scrollY < -200 ? -110 : 0}%)`, borderTopWidth: `${scrollY < -200 ? 0 : 2}px`}} className='w-1/2 h-28 border-t-2 bg-neutral-100 border-purple-400 md:w-96 transition-all duration-500' />
        <div style={{transform: `translateY(${scrollY < -200 ? -100 : 0}%)`, opacity: scrollY > -250 ? 1 : 0}} className="w-full flex flex-col items-center transition duration-500 mb-16">
            <h2 className='font-bold text-2xl md:text-4xl text-neutral-900 flex transition duration-300 -translate-y-16'><span className='font-normal mr-2'>I'm a </span><Typewriter options={{strings: ['Web Developer.', 'Web Designer.', 'UI/UX Designer.'],autoStart: true,loop: true, delay: 150, deleteSpeed: 100}} /></h2>
            <button onClick={() => window.scrollTo({top: document.getElementById("contact").offsetTop})} className='bg-purple-400 bg-opacity-50 px-8 py-3 rounded-full text-xs text-neutral-50 font-bold md:text-base md:py-4 md:px-12 hover:shadow-xl hover:-translate-y-1 transition'>Do You Want to Work With Me?</button>
        </div>
        <div className={`w-full overflow-hidden bg-cover mt-16 transition-all duration-700 ${scrollY < 0 ? "md:h-64 h-16" : "md:h-32 h-8"}`}>
