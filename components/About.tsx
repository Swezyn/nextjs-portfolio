...
export default function About() {
  ...
  return (
    <div id='about' ref={ref} className='flex flex-col items-center overflow-hidden'>
      ...
      <div style={{transform: `scale(${scrollY < 600 && scrollY > -400 ? 100 : 50}%)`, minWidth: "18rem"}} className='mt-8 w-3/5 max-w-screen-lg transition-transform duration-500 items-center md:flex hidden md:flex-col'>
        <div style={{transform: `translateX(${scrollY < 400 && scrollY > -400 ? -50 : 0}%) scale(75%)`}} className='h-72 md:h-96 w-fit aspect-square bg-neutral-700 absolute rounded-xl overflow-hidden transition-transform duration-500'>
          <img src="/me.jpeg" className="w-full h-full object-cover"/>
        </div>
        <div style={{transform: `translateX(${scrollY < 400 && scrollY > -400 ? 30 : 0}%)`}} className='h-72 md:h-96 w-4/6 bg-neutral-100 rounded-xl p-8 shadow-2xl transition-transform duration-500'>
          <p className='text-xs leading-relaxed md:text-xl'><span className='text-sm md:text-2xl font-bold bg-purple-400 py-1 px-6 rounded-xl text-neutral-50 '>Hi!</span> I am a 17 year old boy from Sweden, who loves programming.</p>
      ...
      <div style={{transform: `scale(${scrollY < 600 && scrollY > -400 ? 100 : 50}%) translateY(-30%)`}} className='mt-32 w-4/5 max-w-screen-lg transition-transform duration-500 items-center md:hidden flex flex-col'>
        ...
        <div className='h-72 md:h-96 w-full bg-neutral-100 rounded-xl p-8 shadow-2xl transition-transform duration-500 z-10'>
          <p className='text-xs leading-relaxed md:text-xl'><span className='text-sm md:text-2xl font-bold bg-purple-400 py-1 px-6 rounded-xl text-neutral-50 '>Hi!</span> I am a 17 year old boy from Sweden, who loves programming.</p>
          <p className='overflow-hidden text-xl tracking-widest leading-10 h-10 text-neutral-400'>{[...Array(80)].map((e, i) => ("-"))}</p>
          <p className='text-xs leading-relaxed md:text-xl'>Coding has been a passion of mine for nearly 5 years, and I enjoy creating beautiful and functional websites.</p>
          <p className='overflow-hidden text-xl tracking-widest leading-10 h-10 text-neutral-400'>{[...Array(80)].map((e, i) => ("-"))}</p>
